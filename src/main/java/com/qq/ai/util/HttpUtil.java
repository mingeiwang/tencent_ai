...
public class HttpUtil {
	...
	private static JSONObject httpRequest(String request , String requestMethod , String output){
		...
		try {
			logger.debug("[HTTP]", "http请求request:{},method:{},output{}", request,requestMethod,output);
			//建立连接
			...
			logger.error("[HTTP]", "http请求error:{}", e.getMessage());
		}finally {
			...
			try {
				...
				ex.printStackTrace();
				logger.error("[HTTP]", "http请求error:{}", ex.getMessage());
			}
		}
		return jsonObject;
	} 
